...
import AmountModal from "./TimedepositAmountModal";
  ...
  const MemberAccountModal = ({ openModal, handleDelete, showModal, closeModal }) => {
    ...
    const fetchMembers = useCallback(async () => {
      ...
      try {
        ...
        if (response.data.data.length === 0) {
          setError("No members found without active time deposits."); // Set error for empty result
        } else {
          setError(null); // Clear any previous errors if members are found
        }
    
        setMembers(response.data.data); // Update members state
      } catch (err) {
    ...
    const handleDataUpdate = () => {
      fetchMembers(); // Re-fetch the data
    };
