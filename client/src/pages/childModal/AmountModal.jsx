...
const AmountModal = ({ member, modalType, onClose }) => {
  ...
  const handleTransaction = async () => {
    if (!modalType || (modalType !== "deposit" && modalType !== "withdraw")) {
    ...
    try {
      const endpoint = `${BASE_URL}/timedeposit`;
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg w-96 p-6">
        ...
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700" htmlFor="fixedTerm">
            Fixed Term (Months)
          </label>
          <select
            id="fixedTerm"
        ...
        <div className="mb-4 bg-gray-100 p-4 rounded-md">
          ...
          <p>Maturity Date: <strong>{computation.maturityDate || "-"}</strong></p>
          <p>Interest: <strong>{computation.interest}</strong></p>
          <p>Interest Rate (%): <strong>{computation.interestRate || "-"}</strong></p>
          <p>Payout: <strong>{computation.payout}</strong></p>
        </div>
