...
const MemberProfile = () => {
  ...
  const handleBlurOrEnter = async (field) => {
    try {
      await axios.put(`http://192.168.254.111:3001/api/member/update`, {
        email: user.email,
  ...
  const handleImageUpload = async () => {
    ...
    try {
      const response = await axios.post(
        "http://192.168.254.111:3001/api/member/uploadPicture",
        formData,
        ...
        <>
          {imagePreview || user?.id_picture ? (
            <img
              src={
                imagePreview ||
                `http://192.168.254.111:3001/uploads/${user.id_picture}`
              }
              alt="Profile"
