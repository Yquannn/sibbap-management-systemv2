...
import { FaEdit, FaSearch, FaExclamationTriangle, FaFilter, FaTimes } from "react-icons/fa";
    ...
    >
      <div className="">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-4 md:mb-0">Loan Management</h1>
          ...
          <div className="flex items-center space-x-3">
            ...
            >
              <FaFilter className={isFilterOpen ? "text-blue-600" : "text-gray-500"} />
              <span>{isFilterOpen ? "Hide Filters" : "Show Filters"}</span>
            </button>
            
            {(searchTerm || filterStatus !== "all" || sortField !== "last_name" || sortDirection !== "asc") && (
              <button 
        ...
        {isFilterOpen && (
          <div className="mb-8 bg-white shadow-md rounded-xl p-6 border border-gray-100">
            <h2 className="text-lg font-semibold text-gray-700 mb-4">Advanced Filters</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                ...
                >
                  ...
                  <option value="not-eligible">Not Eligible (Current Borrowers)</option>
                </select>
              </div>
              ...
              <thead className="bg-gray-50">
                <tr>
                  {[
                    ...
                    <th
                      key={column.label}
                      className={`py-4 px-6 text-left text-sm font-semibold text-gray-600 ${column.field ? 'cursor-pointer hover:bg-gray-100' : ''}`}
              ...
              <tbody className="divide-y divide-gray-200">
                {currentMembers.length > 0 ? (
                  currentMembers.map((member, index) => (
                    <tr 
                      key={member.id || index} 
                      className={`hover:bg-blue-50 transition duration-150 ${Number(member.is_borrower) === 1 ? 'bg-gray-50' : ''}`}
                    >
                      <td className="py-4 px-6 text-sm text-gray-700">
                        <span className="">{member.memberCode}</span>
                      </td>
                      ...
                      <td className="py-4 px-6 text-sm text-gray-700">
                        ₱{parseFloat(member.share_capital || 0).toLocaleString('en-US', {minimumFractionDigits: 2})}
                      </td>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50">
            ...
            <div className="relative bg-white rounded-xl shadow-2xl p-8 max-w-md mx-auto transform transition-all">
              <div className="flex flex-col items-center">
                ...
                <h2 className="text-2xl font-bold text-gray-800 mb-2">Not Eligible</h2>
                
                <p className="text-center text-gray-600 mb-6">
                  This member is already a borrower and cannot apply for another loan until the current one is settled.
                </p>
