...
const TransactionForm = ({ member, modalType, onClose }) => {
  ...
  // Use member.availableBalance to initialize balance
  const [balance, setBalance] = useState(parseFloat(member.amount) || 0);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  console.log('member:', member.amount);

  const handleTransaction = async () => {
    const amountValue = parseFloat(amount); // Get user-inputted amount
    ...
    try {
      ...
      if (response.data.success) {
        let updatedBalance = parseFloat(amountValue);
        ...
        alert(
          `${modalType === 'withdraw' ? 'Withdrew' : 'Deposited'} ${amountValue}. New Balance: ${balance}`
        );
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg w-96 p-6">
        ...
        <p className="text-center mb-4">
          Current Balance: <span className="font-bold">{member.amount}</span>
        </p>
