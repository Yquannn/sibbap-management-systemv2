...
import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { FaMoneyCheckAlt, FaUsers, FaCoins, FaClock } from 'react-icons/fa';
...
import ShareCapitalAnalyticsCard from './components/ShareCapitalAnalyticsCard';
import TimeDepositAnalyticsCard from './components/TimeDepositAnalyticsCard';
import TimeDepositMaturityCard from './components/TimeDepositMaturityCard';
...
const SavingsDashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const response = await axios.get('http://localhost:3001/api/savings-summary');
 ...
 // Add a safe trend calculation helper function
const calculateTrendLabel = (value) => {
  if (value === undefined || value === null) return '0.0%';
  return value > 0 ? `+${value.toFixed(1)}%` : `${value.toFixed(1)}%`;
};

// Then update the stats array with null checks
const stats = [
  {
    title: 'Total Savings',
    value: dashboardData.regularSavings,
    trend: calculateTrendLabel(dashboardData.savingsTrend),
    icon: <FaMoneyCheckAlt size={20} className="text-white" />,
    color: 'bg-indigo-500'
  },
  {
  ...
  },
  {
    ...
    trend: calculateTrendLabel(dashboardData.shareCapitalTrend),
    icon: <FaCoins size={20} className="text-white" />,
    color: 'bg-amber-500'
  },
  {
    title: 'Time Deposits',
    value: dashboardData.totalTimedeposit || 0,
    trend: calculateTrendLabel(dashboardData.timeDepositTrend),
    icon: <FaClock size={20} className="text-white" />,
    color: 'bg-blue-500'
  }
];
  ...
  return (
    <div className="bg-gray-50 p-6">
      ...
      <main className="w-full">
        ...
        {activeTab === 'savings' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <SavingsAnalyticsCard data={dashboardData} />
            <MemberDistributionCard data={dashboardData} />
          </div>
        )}

        {activeTab === 'shareCapital' && (
          <div className="grid grid-cols-1 gap-6 mb-6">
            <ShareCapitalAnalyticsCard data={dashboardData} />
          </div>
        )}
        ...
        {activeTab === 'timeDeposit' && (
          <>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              <TimeDepositAnalyticsCard data={dashboardData} />
              <TimeDepositMaturityCard data={dashboardData} />
            </div>
          </>
        )}
      </main>
